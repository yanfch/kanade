...
export const WORKFLOW_AUTHOR_GUIDELINES = [
	...
	"  simple: usually one implement() step, with optional testChange() when clearly justified. For simple tasks, do not use reviewChange(), continueImplementation(), request_human(), or fix loops unless the user explicitly asks for them. If simple validation fails, report the failure instead of authoring an automatic fix loop. For simple docs/text-only edits, do not add reviewChange() unless the user explicitly asks for a reviewer pass.",
	...
	"Use backtick template literals for multi-line prompt strings. Use single or double quotes only for one-line strings. Inside template literals, never include unescaped backticks; write field names as plain text or with regular quotes instead.",
] as const;
...
export function buildWorkflowAuthorPrompt(taskPrompt: string, options: WorkflowAuthorPromptOptions = {}): string {
	...
	const complexity = options.complexityHint;

	// Build the examples section adaptively based on complexity.
	// Simple tasks only need the minimal example; medium/complex get both.
	...
	// Only include the medium review-loop example when complexity is not simple.
	...
	return [
		...
		complexity ? `- Current task complexity hint: ${complexity}.` : null,
		"",
		renderedProfile,
		"",
		...examples,
