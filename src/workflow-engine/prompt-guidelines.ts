...
export const WORKFLOW_AUTHOR_GUIDELINES = [
	...
	"For medium/complex code changes, make testChange() return structured status passed/failed. If validation.status is failed, call continueImplementation() once with the validation feedback, then run testChange() again.",
...
export function buildWorkflowAuthorPrompt(taskPrompt: string, options: WorkflowAuthorPromptOptions = {}): string {
	return [
		...
		"  output: { type: 'object', properties: { status: { type: 'string', enum: ['passed', 'failed'] }, summary: { type: 'string' }, issues: { type: 'array', items: { type: 'string' } } }, required: ['status', 'summary', 'issues'] }",
		"});",
		"let final = candidate;",
